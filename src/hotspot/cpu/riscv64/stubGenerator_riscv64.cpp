...
#define inc_counter_np(counter) \
  ...
  void copy_memory(bool is_aligned, Register s, Register d,
    ...
    __ beqz(count, done);
    __ slli(cnt, count, log2_int(granularity));
    if (is_backwards) {
      __ add(src, s, cnt);
      __ add(dst, d, cnt);
    } else {
  ...
  void verify_oop_array (size_t size, Register a, Register count, Register temp) {
    Label loop, end;
    __ mv(t1, zr);
    __ slli(t0, count, log2_intptr((uintptr_t)size));
    __ bind(loop);
    __ bgeu(t1, t0, end);
  ...
  address generate_conjoint_copy(size_t size, bool aligned, bool is_oop, address nooverlap_target,
    ...
    __ sub(t0, d, s);
    __ slli(t1, count, log2_intptr((uintptr_t)size));
  ...
  address generate_generic_copy(const char* name,
    ...
    int lh_header_size_width = log2_int(Klass::_lh_header_size_mask + 1);
