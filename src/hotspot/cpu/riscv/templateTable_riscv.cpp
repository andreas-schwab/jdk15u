...
void TemplateTable::branch(bool is_jsr, bool is_wide)
{
  ...
  if (UseLoopCounter) {
    ...
    if (TieredCompilation) {
      ...
      __ addi(t0, x10, InvocationCounter::count_increment); // increment counter
      __ sw(t0, Address(t1, be_offset));        // store counter

      __ lwu(x10, Address(t1, inv_offset));    // load invocation counter
      __ andi(x10, x10, (unsigned)InvocationCounter::count_mask_value); // and the status bits
      ...
      if (ProfileInterpreter) {
        ...
        if (UseOnStackReplacement) {
          ...
          __ bltu(x11, t0, dispatch); // Intel == Assembler::below
