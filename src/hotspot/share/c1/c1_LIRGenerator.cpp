...
void LIRGenerator::do_NIOCheckIndex(Intrinsic* x) {
  ...
  if (GenerateRangeChecks) {
    ...
    __ load(generate_address(buf_obj, java_nio_Buffer::limit_offset(), T_INT), left, info);
#else
      ...
      LIR_Opr right = new_register(T_INT);
      __ load(generate_address(buf_obj, java_nio_Buffer::limit_offset(), T_INT), right, info);
#else
